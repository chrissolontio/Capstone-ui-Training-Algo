...
def index():
    ...
    'total': top_category[["Jumlah"]].max().loc["Jumlah"],
    'rev_table' : df2.groupby(["Category", "App"]).agg({"Reviews" : "sum", "Rating" : "mean"}).sort_values(["Reviews"], ascending = False)[:10].reset_index().to_html(classes=['table thead-light table-striped table-bordered table-hover table-sm'])
    }
